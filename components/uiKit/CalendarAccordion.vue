<template>
  <div class="calendar-accordion"
    :class="{'calendar-accordion--open' : open}"
  >
    <div class="calendar-accordion__head"
      @click="open = !open"
    >
    ...
    <div class="calendar-accordion__body">
      <div class="calendar-accordion__data">
        <arrow-prev class="calendar-accordion__data-prev"
          @click.native="prev"
        />
        <span class="timemaster-subtitle">{{ `${computedCurrMonth}, ${currYear}` }}</span>
        <arrow-next class="calendar-accordion__data-next"
          @click.native="next"
        />
      </div>
      <ol class="calendar-accordion__grid"
        :class="{ 'calendar-accordion__grid--prev' : animatePrev, 'calendar-accordion__grid--next' : animateNext }"
      >
        ...
        <li class="calendar-accordion__date out"
          v-for="i in firstDayOfMonth"
          @click="prev"
        >{{ (firstDayOfMonth - lastDateOfLastMonth - i) * -1 }}</li>

        <li class="calendar-accordion__date"
